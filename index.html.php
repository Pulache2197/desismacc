<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="Desismacc Company">

    <link rel="icon" href="img/icon.png" />


    <title>Desismacc | Gol</title>

   
    <link href="css/bootstrap.css" rel="stylesheet">

    <link href="css/cover.css" rel="stylesheet">

    <link href="https://fonts.googleapis.com/css?family=Patua+One" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Exo:500" rel="stylesheet">
    <link rel="stylesheet" href="css/style.css"> 

  </head>

  <body>

      <div class="site-wrapper">
        
        <div class="site-wrapper-inner">
          <div class="hidden-xs -down">
            <div class="cover-container">

              <div class="masthead clearfix">
                <div class="inner">
                  <h3 class="masthead-brand" title="Company Desismacc"><a style="text-decoration: none;"" href="https://pulache2197.github.io/desismacc">Desismacc | Gol</a></h3>
                  <nav class="nav nav-masthead">
                    <a class="nav-link active" href="#">Inicio</a>
                    <a class="nav-link" href="https://pulache2197.github.io/desismacc/nosotros.html">Nosotros</a>
                    <a class="nav-link" href="https://pulache2197.github.io/desismacc/acerca.html">Acerca de Gol</a>
                  </nav>
                </div>
              </div>

              <div class="inner cover">
                <h1 class="cover-heading">Muy pronto, proyecto Gol al Mundo.</h1>
                

                  <div class="container timer_text">
                      <div class="col-md-12">
                          <div class="clock row text-center">
                            <div class="col-md-3 col-sm-3 col-xs-3 text-center" >
                                <div class="timer w3 " id="">01</div>
                                <div class="aits text">Meses</div>
                            </div>
                            <div class="col-md-2 col-sm-2 col-xs-2" >
                                <div class="timer w3" id="days">05</div>
                                <div class="aits text">Dias</div>
                            </div>
                            <div class="col-md-2 col-sm-2 col-xs-2 ">
                                <div class="timer w3layouts " id="hours">10</div>
                                <div class="agileits text">Horas</div>
                            </div>
                         
                            <div class="col-md-2 col-sm-2 col-xs-2">
                                <div class="timer w3las" id="minutes">24</div>
                                <div class="text aits">Minutos</div>
                            </div>
                            
                            <div class="col-md-3 col-sm-3 col-xs-3">
                                <div class="timer stuoyal3w" id="seconds">05</div>
                                <div class="text wthree">Segundos</div>
                            </div>
                          </div>
                      </div>
                  </div>
                  <br>
                <p class="lead">
                  <a href="#" class="btn btn-lg btn-secondary">Ver mas <b style="size:30px;">+</b></a>
                </p>

              </div>

              <div class="mastfoot">
                <div class="inner">
                  <p>Todos los derechos reservados de <a href="#" style="text-decoration: none;"><b>Desismacc Company</b> </a></p>
                </div>
              </div>

            </div>
          </div>
        </div>
      </div>
        
  <script src="https://code.jquery.com/jquery-3.1.1.slim.min.js" integrity="sha384-A7FZj7v+d/sdmMqp/nOQwliLvUsJfDHW+k9Omg/a/EheAdgtzNs3hpfag6Ed950n" crossorigin="anonymous"></script>

  <script  src="js/moment.js"></script>
  <script  src="js/moment-timezone-with-data.js"></script>
  <script  src="js/timer.js"></script>
        
  </body>
</html>
